...
WGIT.options = {
    ...
    args = {
        appearance = {
            ...
            args = {
                showTimerFrame = { type = "toggle", name = "Show Timer Window", order = 1, get = function() return WGIT.db.profile.showTimerFrame end, set = function(_, v) WGIT.db.profile.showTimerFrame = v; if v then WGIT.TimerFrame:Show() else WGIT.TimerFrame:Hide() end end },
                timerLocked = { type = "toggle", name = "Lock Window", order = 2, get = function() return WGIT.db.profile.timerLocked end, set = function(_, v) WGIT.db.profile.timerLocked = v; WGIT:UpdateLockState() end },
                timerOpacity = { type = "range", name = "Opacity", min = 0, max = 1, step = 0.1, order = 3, get = function() return WGIT.db.profile.timerOpacity end, set = function(_, v) WGIT.db.profile.timerOpacity = v; local c = WGIT.db.profile.timerColor; WGIT.TimerFrame:SetBackdropColor(c.r, c.g, c.b, v) end },
            },
        },
        automation = {
            type = "group", name = "Automation", inline = true, order = 2,
            args = {
                autoJoinWG = { type = "toggle", name = "Auto-Join WG Queue", order = 1, get = function() return WGIT.db.profile.autoJoinWG end, set = function(_, v) WGIT.db.profile.autoJoinWG = v; WGIT:UpdateLockButtonText() end },
                autoRaid = { type = "toggle", name = "Auto-Convert Raid", order = 2, get = function() return WGIT.db.profile.autoRaid end, set = function(_, v) WGIT.db.profile.autoRaid = v end },
                autoAccept = { type = "toggle", name = "Auto-Accept (Guild/Friends)", order = 3, get = function() return WGIT.db.profile.autoAccept end, set = function(_, v) WGIT.db.profile.autoAccept = v end },
                autoRelease = { type = "toggle", name = "Auto-Release in BG", order = 4, get = function() return WGIT.db.profile.autoRelease end, set = function(_, v) WGIT.db.profile.autoRelease = v end },
...
local function CreateStyledTimer()
    ...
    -- Right-click toggles window lock
    f:SetScript("OnMouseUp", function(self, button)
        if button == "RightButton" then
            WGIT.db.profile.timerLocked = not WGIT.db.profile.timerLocked
            WGIT:UpdateLockState()
            local state = WGIT.db.profile.timerLocked and "|cffff0000Locked|r" or "|cff00ff00Unlocked|r"
            WGIT:Print("Window " .. state)
        end
    end)
    ...
    f.rb:SetScript("OnMouseDown", function() if not WGIT.db.profile.timerLocked then f:StartSizing() end end)
    ...
    f.toggle:SetScript("OnClick", function() WGIT.db.profile.autoJoinWG = not WGIT.db.profile.autoJoinWG; WGIT:UpdateLockButtonText() end)
...
function WGIT:OnInitialize()
    ...
    local LDB = LibStub("LibDataBroker-1.1"):NewDataObject("WGIT", {
        type = "data source", text = "WGIT", icon = "Interface\\Icons\\Ability_Warrior_OffensiveStance",
        OnClick = function() WGIT:OpenConfig() end,
        OnTooltipShow = function(tt)
            tt:AddLine("|cff00ff00WGIT|r |cffaaaaaa- Wintergrasp Invite Tools v1.0|r")
            tt:AddLine(" ")
            local wgText, active, urgent = GetWGTimer()
            if active then
                tt:AddDoubleLine("Wintergrasp:", "|cff00ff00Active|r")
            elseif urgent then
                tt:AddDoubleLine("Wintergrasp:", "|cffff4400" .. wgText .. "|r")
            else
            ...
            tt:AddLine("|cffaaaaaa Left-click to open settings|r")
        end,
    })
    LibStub("LibDBIcon-1.0"):Register("WGIT", LDB, self.db.profile.minimap)
    LibStub("LibDBIcon-1.0"):Show("WGIT")
end

function WGIT:OnEnable()
...
-- Auto-reset on zone entry removed; use the Reset Stats button on the frame instead
function WGIT:ZONE_CHANGED_NEW_AREA()
end
...
function WGIT:BATTLEFIELD_MGR_QUEUE_INVITE()
...
function WGIT:PARTY_INVITE_REQUEST(_, sender)
